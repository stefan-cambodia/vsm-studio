...
écoute au clic, et toute la logique testée hors JUCE. Total : **916 tests moteur** (98 core + 672 audio
+ 124 interchange + 11 CLAP + 11 façades,
...
### Bilan actuel : 916 tests moteur + 18 tests d'analyse, tous verts
...
## 44. La flûte : ce que je croyais être une oscillation était du CONTINU

**UNE AFFIRMATION DE CETTE MÊME SECTION EST RETIRÉE, ET C'EST LE PLUS UTILE DE
CE QUI SUIT.** Une première rédaction annonçait ici que le modèle de flûte
« s'auto-oscille — niveau efficace 0,27 à 0,64, note qui tient, spectre riche »
et qu'il ne lui manquait que la justesse. **C'était faux**, et c'est une mesure
ultérieure, plus fine, qui l'a montré. Le paragraphe est corrigé plutôt
qu'effacé : c'est exactement le défaut que ce document reproche à une session
antérieure, qui avait écrit du prototype conique qu'il « sonne juste » sans
jamais l'avoir mesuré.

**CE QUI S'EST RÉELLEMENT PASSÉ.** Le modèle jet/tuyau produisait bien un signal
fort (niveau efficace 0,27) dont l'autocorrélation trouvait une période propre,
et dont on pouvait même corriger la justesse à quelques cents près. Mais
l'analyse spectrale a fini par poser la seule bonne question — *où est
l'énergie ?* — et la réponse était : **à 0 Hz**. La composante dominante était
une composante CONTINUE. Chaque harmonique pesait 0,00004 quand le niveau global
valait 0,27 : l'instrument sonnait « fort » sans qu'aucun rang ne soit audible.

**LA CAUSE EST STRUCTURELLE, ET ELLE EST INSTRUCTIVE.** Une boucle NON
INVERSANTE fermée sur un passe-bas donne au continu le plus fort gain de tous :
il s'accumule jusqu'à tout écraser. C'est propre à cette topologie -- la boucle
inversante de `vsm.wind` ne peut pas avoir ce problème -- et c'est aussi
physiquement interdit : un tuyau ouvert aux deux bouts a une pression nulle à
ses extrémités, il ne peut pas porter de continu.

**ET AVEC LE BLOQUEUR DE CONTINU, QUI EST OBLIGATOIRE, IL NE RESTE RIEN.** Le
niveau efficace tombe de 0,27 à 0,005, et la fréquence produite se fige à
1 412 Hz quelle que soit la note demandée. Balayé sur quatre gains de boucle
(0,5 à 2,0) et trois amortissements : **la boucle ne s'entretient à aucun
réglage**. Sans bruit d'amorçage, elle ne démarre même pas.

**CE QUE ÇA APPREND, ET QUI VAUT PLUS QUE LA MACHINE.** L'obstacle est donc le
MÊME que celui du prototype conique — un gain de boucle sous le seuil — atteint
par une topologie entièrement différente. Quatre routes à anche, puis une route
à jet : le problème n'est pas la forme de la perce ni la nature de l'excitateur,
c'est la formulation du COUPLAGE entre l'excitateur et la colonne d'air. La
prochaine tentative devra commencer par là, et non par une cinquième topologie.

**DEUX DÉFAUTS RÉELS CORRIGÉS EN CHEMIN, ET ILS RESTENT DANS LE SOURCE.** La
table du jet bornait son entrée à ±1 avant d'appliquer `x - x³`, or ce polynôme
S'ANNULE en ±1 : soufflée à fond, la machine ne rendait plus rien. Et le filtre
de boucle avait une coupure fixe, ce qui laissait la boucle choisir son mode
toute seule -- avec une coupure proportionnelle à `f0`, elle ne partait plus
...
et une autocorrélation trouve une période même dans un signal qui n'en a pas de
