...
def mesurer_separation(verite: dict, morceau: Path, stems_separes: Optional[Path]) -> dict:
    ...
    fichiers = {f.stem: f for f in sorted(stems_separes.rglob("*.wav"))}
    if not fichiers:
...
def mesurer_morceau(morceau: Path, course: Path, stems_separes: Optional[Path],
    ...
    # Les cibles que la chaîne a jugées : les stems qu'elle a reçus.
    ...
    if stems_vrais_fournis or stems_separes is None or not stems_separes.exists():
        ...
        cibles = {f.stem: lire_wav_mono(f) for f in sorted(stems_separes.rglob("*.wav"))}
    ...
    return {
        ...
        "arbitrage": arbitrage, "global": global_,
    }
