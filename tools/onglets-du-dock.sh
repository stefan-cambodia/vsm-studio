...
for langue in fr en; do
    for verbe in mixer automation effets midi-cc liste tempo; do
        ...
        env HOME="$maison" VSM_LANGUE="$langue" VSM_DELAI=2000 \
...
env HOME="$maison" VSM_VUE="?" VSM_DELAI=1500 VSM_CAPTURE="$brouillon/verbes.png" \
...
if [ -n "$simples" ]; then
    ...
    env HOME="$maison2" VSM_VUE="$simples" VSM_DELAI=2500 \
