...
/// LE BIQUAD DU MOTEUR (`dsp::Biquad`), et non une copie : deux passe-bas
/// écrits à deux endroits finiraient par ne plus couper à la même fréquence.
/// Q de Butterworth, gain nul.
struct Bande {
...
std::vector<int64_t> detectOnsets(const std::function<bool(int64_t, float&, float&)>& frameAt,
    ...
    auto monteeLaPlusRaide = [&](int64_t trame, int bande) {
        ...
        using Type = vsm::audio::dsp::Biquad::Type;
        Bande f1(bande == 1 ? Type::LowPass : Type::HighPass, bande == 3 ? 2000.0f : 200.0f, sampleRate);
        Bande f2(Type::LowPass, 2000.0f, sampleRate);
