...
| ~~5~~ | ~~Des sous-oscillateurs de niveau varié sur les basses~~ **RETIRÉE le 13/09 (D278)** : l'exigence était mal conçue — seules **trois machines du parc** exposent `oscillator.sub.level`, aucun tirage ne produira la variété demandée sans biaiser le choix des machines. Et l'hypothèse n'avait pas besoin d'un corpus : un TÉMOIN suffit. Mesurée en une minute par `tools/sous-oscillateur.py`, elle est **confirmée** — la part de notes une octave trop bas passe de **0,0 % à 75,0 %** quand le sous-oscillateur monte de 0,0 à 0,8 | D269, tranchée par D278 | plus rien à demander au corpus de ce côté |
