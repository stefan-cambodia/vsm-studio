...
**64 machines · 16 effets · 1 146 paramètres nommés · 1 961 tests verts**
...
**Importer un MIDI dans le projet.** *Fichier ▸ Importer un MIDI dans le projet…* — ou un fichier `.mid` lâché sur la fenêtre — ajoute ses pistes à la suite des vôtres, posées à la tête de lecture et converties à la résolution du projet ; son tempo et ses mesures sont ignorés (le projet garde les siens, et cela vous est dit). *Ouvrir MIDI…*, lui, remplace le projet. **Dans les deux cas** (D305-D307) : un fichier de **format 0** — une seule piste pour seize canaux, le format des banques de MIDI grand public — est **découpé en une piste par canal** (« Mixdown · canal 3 »), chaque piste reçoit un **clip** sur son matériau, et **une machine d'après son programme General MIDI** — le canal 10 un kit de batterie, un « Synth Bass 1 » la TB-303, un « Pad 2 (warm) » le Jupiter-8, un fichier sans programme **le nom de la piste** quand il parle (« bass », « strings », « pad », « Batterie »… — D313), sinon le piano, comme le veut la norme — et **quand une banque General MIDI est installée** (`tools/installer-banques-midi.py`), c'est elle qui rend le programme, par `vsm.multisample` et son profil (D309) ; la machine du parc reste le repli. Chaque choix est écrit au journal avec le nom GM ; vous changez ensuite ce que vous voulez dans la liste des pistes. Une piste que le fichier ne nomme pas s'appelle « Piste N » (D308), le même nom dans la liste, l'arrangement, le clip et la console. **Dans l'autre sens, *Exporter MIDI…* écrit un programme General MIDI par piste** (D312) : celui réglé pour le matériel s'il y en a un, sinon celui qui désigne la machine de la piste (TB-303 → 38, piano → 0, boîte à rythmes → un kit au canal 10) ; un fichier ouvert garde ses propres programmes, et une machine sans équivalent GM n'en écrit pas — le journal le dit. À l'ouverture comme à l'import, chaque piste MIDI reçoit ses clips aux bornes de ses notes, à la mesure près, et un silence de huit mesures en sépare deux (D333-D334) : l'arrangement montre où chaque partie entre et s'arrête, sans ouvrir une note.

**Les locateurs sur la sélection.** Choisissez des clips (ou des notes dans le piano roll) et appuyez sur **P** : la boucle prend leur étendue et s'active — le geste de Cubase. Le menu du clip zoome aussi : *Zoom : tout voir*, *Zoom : la sélection*, et **Zoom : tout voir (les deux vues)** — le nom que porte Ctrl+0 dans la fenêtre des raccourcis depuis D355 — vaut pour les deux vues à la fois.
...
où elle ne faisait avant que se jouer. **La tête de lecture traverse ces lanes** — automation, MIDI CC, tempo — d'un trait ambre, le même que dans l'arrangement et le piano roll (D285), **et les lanes suivent la fenêtre de l'arrangement** (D286) : même tick, même colonne d'écran, au pixel — ce qu'on voit là-haut et ce qu'on règle en bas se lisent à la même verticale, et le zoom ou le défilement de l'arrangement entraînent les lanes. Quand c'est le piano roll qui est en haut, les lanes suivent SA fenêtre — la colonne des notes, clavier déduit (D326), comme les lanes de contrôleur de Cubase ; ni l'un ni l'autre visible, une lane montre le morceau entier sur sa largeur. **Les onglets *Automation*, *MIDI CC*, *Effets* et *Liste* suivent la piste choisie** dans la liste des pistes (D303) ; choisir une autre piste dans la liste déroulante d'un onglet n'y change rien, pour pouvoir regarder l'automation d'une piste en jouant une autre au clavier. **Une lane d'automation suit l'échelle de son paramètre** (D327, D345) : une
...
**L'ordre de la liste** (D289) : les machines d'abord, puis les presets, les profils et les échantillons ; à nature égale, le dossier du projet avant la bibliothèque ; et à origine égale, le nom, sans casse. Une recherche garde cet ordre. **La nature est un mot de la recherche** (D290) : `303 preset` ne montre que des presets, `échantillon kick` que des fichiers audio — le filtre par type de Cubase et de Live, sans bouton de plus.
...
logarithmique, une courbe reste une courbe.
...
dépôt en contient vingt-quatre — arrêtait tout banc. `n` est le rang affiché, à
partir de 1 ; un rang hors liste ne charge rien **et le dit**, plutôt que de
prendre le premier venu — un banc qui prend le premier mesure une machine que
personne n'a demandée, et sa capture est vraisemblable et fausse.
...
`VSM_PISTE_CHOISIE` (D369) écrit la piste choisie et combien il y en a (« 2 sur 3 »), avec le relevé du piano roll. Aucun relevé ne le disait : `VSM_GESTE : choisir` ne parle que du geste qu'on vient de jouer, si bien qu'une piste choisie par le PROJET à l'ouverture ne laissait aucune trace. Depuis la même phase, `VSM_PIANOROLL_ZONES=1` dit aussi la **fenêtre de hauteurs** du piano roll — `haut=`, `bas=`, `lignes=`, `zoom=`, `defilement=` — et non plus le seul rang : un projet se rouvre désormais sur la piste et l'éditeur qu'on a laissés, et sans ces chiffres cela ne se mesure pas.
