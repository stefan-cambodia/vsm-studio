...
| A21 | **Aucun réglage de machine ne s'annule** — trouvé en vérifiant D140 : double-clic puis Ctrl+Z, le volume de la ligne et le fader reviennent (-0.9 dB), la coupure d'une façade reste à 800 Hz ; un glissé sur une façade ne s'annule pas davantage (son rappel envoie la valeur au synthé sans ouvrir de pas d'annulation). Ces réglages font partie du projet (`instruments/track_NN.synth.json`) ; D36 a compté les champs de la piste qui échappaient à l'annulation, pas les réglages de machine | `ROADMAP-daw.md` D140 | réglage de façade + Ctrl+Z : 800 Hz → **800 Hz** |
