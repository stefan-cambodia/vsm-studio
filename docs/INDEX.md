...
| A24 | **L'activation du bus master ne s'annule pas** — mesuré par D145 : « Master Enabled » enregistré à 1, un clic sur le bouton MASTER l'éteint (moteur 0.00), et Ctrl+Z le laisse éteint ; un geste de piste intercalé n'y change rien, puisque depuis D144 le pas photographie le MASTER du moteur — donc l'activation DÉJÀ éteinte. C'est un paramètre du projet (`project.json`, « Master Enabled », `MasterBus.h:56`), à la différence de l'écoute mono, qui survit à l'annulation comme elle le doit (D23.5, vérifié au même banc : mono 1, zéro clé mono au relevé). Cause : `onMasterEnable` (`MainComponent.cpp:325`) pose la valeur dans le moteur sans ouvrir de pas ni prévenir le modèle — le défaut de D143, pour le seul rappel que D144 n'a pas repris. Se corrige dans l'application seule | `ROADMAP-daw.md` D145 | clic MASTER puis Ctrl+Z : moteur **0.00** (attendu 1.00) |
