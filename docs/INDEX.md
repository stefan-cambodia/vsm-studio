...
| A11 | **Annuler ne s'atteint que depuis le piano roll** — le menu Édition n'a ni « Annuler » ni « Rétablir », et Ctrl+Z n'agit que si le piano roll a le focus (`MainComponent::keyPressed` ne connaît pas `EditUndo`). Trouvé parce que le banc de D76 n'a pas su annuler : son cas « annulation » a enregistré 3 pistes, c'est-à-dire un second ajout | `ROADMAP-daw.md` D76 | 0 entrée de menu ; Ctrl+Z : `VSM_TOUCHE : touche inconnue ou sans commande` |
| A9 | **L'interface n'est traduite qu'en partie** — la barre de menus l'est entièrement (227/227), mais les panneaux du dock, les fenêtres flottantes et les phrases fabriquées dans `interchange/` restent en français. Une chaîne sans traduction ressort en français, jamais vide | `ROADMAP-daw.md` D73 → D74 → D77 → D78 | **321** paires dans la table ; barre de menus 227/227, barre du piano roll, panneau des effets 31/31. **La bascule en direct rend désormais la même image que le démarrage** (0 pixel sur 937 888, D78 ; 4 056 à D77). Reste vu : « RÉGLAGES », titre de section ajouté par l'application sur une façade anglaise ; et les fenêtres flottantes, les menus contextuels de l'arrangement, les phrases de `interchange/` ; ~1 000 chaînes affichées au total |
