...
| A9 | **L'interface n'est traduite qu'en partie** — la barre de menus l'est entièrement (227/227), mais les panneaux du dock, les fenêtres flottantes et les phrases fabriquées dans `interchange/` restent en français. Une chaîne sans traduction ressort en français, jamais vide | `ROADMAP-daw.md` D73 → D74 → D77 → D78 → D80 → D81 → D82 → D83 | **583** paires dans la table. **Les noms des gestes** (« Undo: … », fenêtre d'historique) : 143 / 143, vérifiés à l'écran (D82). **Menus de l'arrangement** : 70 entrées identiques FR / EN → 6, neutres (D83), listés par `VSM_MENU_LISTE`. Le volet du rapport d'ouverture suit la bascule (D84 : 3 482 → 0 px, et rouvert après la bascule, dans la nouvelle langue). La fenêtre des **Préférences** est traduite et suit la bascule (D85 : français et anglais étaient la même image, 0 px ; 23 384 px après). **Assemblage des prises et ordre de jeu** traduits et suivant la bascule (D86 : 0 px FR / EN avant, rien n'était traduit). **Raccourcis, associations MIDI, navigateur, reconstruction** traduits (D87 : trois fenêtres qui étaient la même image dans les deux langues ; la reconstruction non photographiée). Table à **688** paires. **Les phrases du moteur dans le rapport d'ouverture** sont traduites à l'affichage par 29 modèles (D89 : lignes identiques FR / EN 22 → 9 sur trois projets, les 9 étant noms de dossier et lignes vides ; aucun mot français restant). Reste : le même traducteur à brancher sur les messages d'export, les réserves de preset, les rapports d'import et de reconstruction ; les descriptions des machines et la source « Parc VSM » du navigateur (chaînes du moteur), (la colonne de source coupée par le bord est corrigée par D88, avec le survol qui éclaircissait toute la liste), les phrases fabriquées dans `interchange/`, les rapports d'import et de reconstruction. **La barre de menus est traduite entièrement, mesuré sur ce qu'elle affiche** (`VSM_MENU_LISTE`) : 150 entrées identiques en français et en anglais avant D80, 69 après D80, **35 après D81 — toutes neutres** (chemins, ports, « 150 % », « Legato »…). Le « 227 / 227 » de D73 était faux : 26 paires existaient sans que le code appelle `tr()`. La bascule en direct rend la même image que le démarrage (0 pixel, D78). Reste : les fenêtres flottantes, les menus contextuels de l'arrangement, le nom du geste dans « Undo: … » ; « RÉGLAGES », titre de section ajouté par l'application sur une façade anglaise ; et les fenêtres flottantes, les menus contextuels de l'arrangement, les phrases de `interchange/` ; ~1 000 chaînes affichées au total |
