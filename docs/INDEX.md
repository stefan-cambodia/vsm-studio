...
| A5 | **Trente et une façades sur 63 demandent plus de largeur que le rack.** `vsm.generic` (1 372 px réclamés) et `vsm.obx` (1 148) sont **corrigées par D65** : blocs empilés sur huit colonnes, 0 px → 35 px. `vsm.wavetable`, `vsm.pcmhybrid` et `vsm.supersaw` ont la même forme — tous leurs blocs sur une rangée — mais **attendent leur mesure à l'écran** avant d'être touchées | `ROADMAP-daw.md` D64, D65 | médiane 420 px, min 156, max 1 372 |
