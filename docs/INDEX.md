...
| A5 | **Onze façades sur 63 réclament encore plus de largeur que le rack, PAR LE CALCUL.** Dix ont été mesurées à l'écran et corrigées — `vsm.generic` et `vsm.obx` (D65), puis six autres plus le Chebyshev et l'ARP (D66) : blocs empilés sur huit colonnes, **67 boutons sous 18 px → 0**. Les onze restantes n'ont pas été mesurées ; on ne corrige pas une façade sur la foi d'un calcul | `ROADMAP-daw.md` D64 → D66 | médiane 420 px, min 156, max 1 372 |
