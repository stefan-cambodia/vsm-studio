...
| A22 | **À l'ouverture, la tranche MASTER montre les valeurs d'usine au lieu de celles du projet** — mesuré par D141 : LOW enregistré à +6 dB, le moteur a 6.00, le bouton affiche 0.0 dB. Cause : dans `rebuildFromProject`, le mixeur relit le MASTER dans le moteur (`mixer_.setProject`) AVANT que le MASTER du projet y soit appliqué. Conséquence : un double-clic « remettre à zéro » ne fait rien (le bouton est déjà à 0), pendant que +6 dB s'applique ; l'affichage ne se rattrape qu'à la reconstruction suivante (une annulation). Se corrige dans l'application seule. Ensuite seulement se mesurera le soupçon de D141 (une annulation qui ramène le MASTER au dernier enregistrement) | `ROADMAP-daw.md` D141 | bouton 0.0 dB / moteur 6.00 |
