...
| A7 | **L'écran de rapport d'ouverture ne se photographie qu'une fois sur sept** (`VSM_CAPTURE_PANNEAUX` contre `showMessageBoxAsync` : une course, pas un délai trop court). La preuve d'ouverture passe donc par la sortie d'erreur, qui est déterministe | `ROADMAP-daw.md` D71 | 1 capture / 7 tentatives, 1 800 à 3 000 ms |
