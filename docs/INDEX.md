...
| A20 | **Au dock des pistes le plus étroit (180 px, le minimum permis), le nom de la sortie d'une ligne ne se lit pas** (« -… ») — le reste de D136, BORNÉ par une mesure : au défaut du code (300 px), la disposition étroite se lit entièrement (« TB-303-style Acid Synth », M S R, « -> Master ») ; à 180 px, ~150 px utiles ne tiennent pas trois boutons ET le nom d'un bus. Les préférences de l'utilisateur sont à 180 : c'est ce qu'il voit. La disposition est réglable, et on ne réécrit pas sa largeur | `ROADMAP-daw.md` D136 | sortie : « -… » à 180 px ; lisible à 300 |
